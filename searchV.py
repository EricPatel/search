...
def dfs(graph):
    ...
    while len(stack) != 0:
        ...
        if x == dim - 1 and y == dim - 1:
            ...
            return getPath(prevMap, (dim - 1, dim - 1)), visited, end - start
        else:
            ...
            if checkPoint(i, j, dim) and visited[i][j] == -1:
                while len(fQueue) != 0:
                    ...
                    if(i == currFX and j == currFY):
                        ...
                        nodes = map.printVisited(visited, graph, 0)
                        return path + currPoint[2][::-1], visited, end - start
...
def aStar(graph, heuristicMethod):
    ...
    while len(heap) != 0:
        ...
        if x == dim - 1 and y == dim - 1:
            ...
            return getPath(prevMap, goal_cell), visited, end - start
        else:
            # Generate a list of all possible neighboring points from the current point (x,y)
            points = [(x, y-1), (x,y+1), (x-1, y), (x+1, y)]
            for (i,j) in points:
